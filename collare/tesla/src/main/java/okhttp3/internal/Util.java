...
public final class Util {
    ...
    public static boolean discard(t tVar, int i, TimeUnit timeUnit) {
        try {
            return skipAll(tVar, i, timeUnit);
        } catch (IOException unused) {
            return false;
        }
    }

    public static boolean skipAll(t tVar, int i, TimeUnit timeUnit) {
        long nanoTime = System.nanoTime();
        long deadlineNanoTime = tVar.timeout().hasDeadline() ? tVar.timeout().deadlineNanoTime() - nanoTime : Long.MAX_VALUE;
        tVar.timeout().deadlineNanoTime(Math.min(deadlineNanoTime, timeUnit.toNanos((long) i)) + nanoTime);
        try {
            c cVar = new c();
            while (tVar.read(cVar, 8192) != -1) {
                cVar.u();
            }
    ...
    public static Charset bomAwareCharset(e eVar, Charset charset) {
        if (eVar.a(0, UTF_8_BOM)) {
            eVar.i((long) UTF_8_BOM.h());
            return UTF_8;
        } else if (eVar.a(0, UTF_16_BE_BOM)) {
            eVar.i((long) UTF_16_BE_BOM.h());
            return UTF_16_BE;
        } else if (eVar.a(0, UTF_16_LE_BOM)) {
            eVar.i((long) UTF_16_LE_BOM.h());
            return UTF_16_LE;
        } else if (eVar.a(0, UTF_32_BE_BOM)) {
            eVar.i((long) UTF_32_BE_BOM.h());
            return UTF_32_BE;
        } else if (!eVar.a(0, UTF_32_LE_BOM)) {
            return charset;
        } else {
            eVar.i((long) UTF_32_LE_BOM.h());
            return UTF_32_LE;
        }
    }
