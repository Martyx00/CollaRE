...
public class X509CertificateObject extends X509Certificate implements PKCS12BagAttributeCarrier {
    ...
    public void checkValidity(Date date) {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f6420c.getEndDate().getTime());
        } else if (date.getTime() < getNotBefore().getTime()) {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f6420c.getStartDate().getTime());
        }
    }

    public int getVersion() {
        return this.f6420c.getVersionNumber();
    }

    public BigInteger getSerialNumber() {
        return this.f6420c.getSerialNumber().getValue();
    }

    public Principal getIssuerDN() {
        try {
            return new X509Principal(X500Name.getInstance(this.f6420c.getIssuer().getEncoded()));
    ...
    public X500Principal getIssuerX500Principal() {
        try {
            ...
            new ASN1OutputStream(byteArrayOutputStream).writeObject(this.f6420c.getIssuer());
    ...
    public Principal getSubjectDN() {
        return new X509Principal(X500Name.getInstance(this.f6420c.getSubject().toASN1Primitive()));
    }

    public X500Principal getSubjectX500Principal() {
        try {
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            new ASN1OutputStream(byteArrayOutputStream).writeObject(this.f6420c.getSubject());
            return new X500Principal(byteArrayOutputStream.toByteArray());
        } catch (IOException unused) {
            throw new IllegalStateException("can't encode issuer DN");
        }
    ...
    public Date getNotBefore() {
        return this.f6420c.getStartDate().getDate();
    }

    public Date getNotAfter() {
        return this.f6420c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public byte[] getTBSCertificate() {
        try {
            return this.f6420c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
    ...
    public byte[] getSignature() {
        return this.f6420c.getSignature().getOctets();
    }
    ...
    public String getSigAlgOID() {
        return this.f6420c.getSignatureAlgorithm().getAlgorithm().getId();
    }

    public byte[] getSigAlgParams() {
        if (this.f6420c.getSignatureAlgorithm().getParameters() == null) {
            return null;
        }
        try {
            return this.f6420c.getSignatureAlgorithm().getParameters().toASN1Primitive().getEncoded(ASN1Encoding.DER);
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public boolean[] getIssuerUniqueID() {
        DERBitString issuerUniqueId = this.f6420c.getTBSCertificate().getIssuerUniqueId();
    ...
    public boolean[] getSubjectUniqueID() {
        DERBitString subjectUniqueId = this.f6420c.getTBSCertificate().getSubjectUniqueId();
    ...
    public Set getCriticalExtensionOIDs() {
        ...
        Extensions extensions = this.f6420c.getTBSCertificate().getExtensions();
        if (extensions == null) {
            return null;
        }
    ...
    public boolean hasUnsupportedCriticalExtension() {
        ...
        if (getVersion() != 3 || (extensions = this.f6420c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration oids = extensions.oids();
        while (oids.hasMoreElements()) {
    ...
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f6420c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public byte[] getEncoded() {
        try {
            return this.f6420c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public final void verify(PublicKey publicKey) {
        Signature signature;
        String signatureName = X509SignatureUtil.getSignatureName(this.f6420c.getSignatureAlgorithm());
        try {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature) {
        if (isAlgIdEqual(this.f6420c.getSignatureAlgorithm(), this.f6420c.getTBSCertificate().getSignature())) {
            X509SignatureUtil.setSignatureParameters(signature, this.f6420c.getSignatureAlgorithm().getParameters());
